...
public class LichMinionsGoal extends Goal {
    ...
    public void tick() {
        ...
        float dist = this.lich.distanceTo(target);
        boolean canSee = this.lich.getSensing().hasLineOfSight(target);
        ...
        if (this.strafingTime > -1) {
            if (dist > ATTACK_RANGE * 0.75F) this.strafingBackwards = false;
            else if (dist < ATTACK_RANGE * 0.25F) this.strafingBackwards = true;
        ...
        if (this.lich.getAttackCooldown() == 0) {
            ...
            } else if (dist < ATTACK_RANGE && this.lich.getSensing().hasLineOfSight(target)) {
                this.lich.launchProjectileAt(this.lich.getNextAttackType() == 0 ? new LichBolt(this.lich.level(), this.lich) : new LichBomb(this.lich.level(), this.lich));
                this.lich.swing(InteractionHand.MAIN_HAND);
    ...
    private void spawnMinionAt() {
        ...
        if (minionSpot != null && this.lich.level() instanceof ServerLevelAccessor accessor) {
            ...
            minion.moveTo(minionSpot.x(), minionSpot.y(), minionSpot.z(), this.lich.getYRot(), 0.0F);
