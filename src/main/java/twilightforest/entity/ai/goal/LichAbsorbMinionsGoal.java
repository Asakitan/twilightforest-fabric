...
public class LichAbsorbMinionsGoal extends Goal {
    ...
    public void tick() {
        ...
        List<LichMinion> minions = this.lich.level().getEntitiesOfClass(LichMinion.class, this.lich.getBoundingBox().inflate(32.0D, 16.0D, 32.0D), minion -> minion.getMaster() == this.lich);
        if (minions.isEmpty()) {
            return;
        }
