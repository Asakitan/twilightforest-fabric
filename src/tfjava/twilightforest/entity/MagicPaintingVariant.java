...
/**
 * 1:1 port of upstream {@code twilightforest.entity.MagicPaintingVariant} —
 * datapack-registry-backed record describing a single Magic Painting variant
 * (a layered scrolling-parallax composite). Each variant has width/height (in
 * units of 16px tiles), an ordered layer list, an author component, and a
 * back-side sprite. The {@link Layer} inner record adds parallax /
 * opacity-modifier hooks, {@link Layer.Parallax}, and {@link Layer.OpacityModifier}
 * for the various dynamic-tinting modes (DAY_TIME / WEATHER / HOLDING_ITEM /
	...
	public record Layer(String path, @Nullable Parallax parallax, @Nullable OpacityModifier opacityModifier, boolean fullbright, boolean localLighting) {
		...
		public record OpacityModifier(Type type, float multiplier, boolean invert, float min, float max, float from, float to, ItemStack item, Optional<MobEffectCategory> effectCategory) {
			...
			public enum Type implements StringRepresentable {
				...
				private final boolean toThePowerOfItsMultiplier;
